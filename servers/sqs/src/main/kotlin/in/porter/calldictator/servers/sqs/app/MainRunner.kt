package `in`.porter.calldictator.servers.sqs.app

import `in`.porter.kotlinutils.serde.jackson.json.JsonMapper
import `in`.porter.calldictator.api.models.async.AsyncJob
import `in`.porter.calldictator.servers.sqs.configs.customConfigure
import `in`.porter.calldictator.servers.sqs.consumers.AsyncSQSConfig
import `in`.porter.calldictator.servers.sqs.di.SQSComponentFactory
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.launch
import kotlinx.coroutines.withContext
import org.apache.logging.log4j.kotlin.logger

private val logger = logger("in.porter.calldictator.servers.sqs.app.MainRunnerKt")
